...
def parser():
 p=argparse.ArgumentParser();p.add_argument("--db",default=str(DEFAULT_DB));p.add_argument("--neurons",default=str(DEFAULT_NEURONS));s=p.add_subparsers(dest="command",required=True);s.add_parser("seed");t=s.add_parser("train");t.add_argument("--rounds",type=int,default=8);t.add_argument("--report",default="data/training_report.json");c=s.add_parser("chat");c.add_argument("text",nargs="*");a=s.add_parser("add");a.add_argument("term");a.add_argument("--category",default="general");a.add_argument("--teaching");r=s.add_parser("relate");r.add_argument("source");r.add_argument("relation");r.add_argument("target");u=s.add_parser("remember");u.add_argument("key");u.add_argument("value");return p
def main():
