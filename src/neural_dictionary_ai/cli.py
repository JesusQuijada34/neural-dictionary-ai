...
from .agent import Agent, DesktopOrganizer
...
def parser():
 p=argparse.ArgumentParser();p.add_argument("--db",default=str(DEFAULT_DB));p.add_argument("--neurons",default=str(DEFAULT_NEURONS));s=p.add_subparsers(dest="command",required=True);s.add_parser("seed");t=s.add_parser("train");t.add_argument("--rounds",type=int,default=8);t.add_argument("--report",default="data/training_report.json");e=s.add_parser("export");e.add_argument("path");i=s.add_parser("import");i.add_argument("path");s.add_parser("skills");ag=s.add_parser("agent");ag.add_argument("text",nargs="+");ag.add_argument("--root",default=".");o=s.add_parser("organize");o.add_argument("root");o.add_argument("--apply",action="store_true");d=s.add_parser("debate");d.add_argument("topic");d.add_argument("options",nargs="+");c=s.add_parser("chat");c.add_argument("text",nargs="*");a=s.add_parser("add");a.add_argument("term");a.add_argument("--category",default="general");a.add_argument("--teaching");r=s.add_parser("relate");r.add_argument("source");r.add_argument("relation");r.add_argument("target");u=s.add_parser("remember");u.add_argument("key");u.add_argument("value");return p
def main():
 ...
 try:
  ...
  elif args.command=="agent": print(json.dumps(Agent().plan(" ".join(args.text),args.root),ensure_ascii=False,indent=2))
  elif args.command=="organize":
   organizer=DesktopOrganizer(); result=organizer.apply(args.root) if args.apply else {"mode":"plan","actions":organizer.plan(args.root),"requires_confirmation":True}; print(json.dumps(result,ensure_ascii=False,indent=2))
  elif args.command=="debate": print(json.dumps(Agent().decide(args.topic,args.options),ensure_ascii=False,indent=2))
  elif args.command=="skills": print(json.dumps(SkillRegistry(ROOT/"skills/registry.yml").list(),ensure_ascii=False,indent=2))
  else:
